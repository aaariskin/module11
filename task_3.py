...
size_file = int(input('Укажите размер файла для скачиваничя: '))
connect_speed = int(input('Какова скорость вашего соединения? '))

second = 1
percent = 0

for files in range(1, size_file + 1):
    if files == connect_speed * second or files == size_file:
        percent = math.ceil((files / size_file) * 100)
        print(
            f'Прошло {second} сек. Скачано {files} из {size_file} Мб ({percent}%)'
        )
        second += 1
